...
void rate()
    {
        float speed = cm/duration;
        for(float i=100-percent; i<=50; i+=10)
        {
            if(speed<=(pump+0.02) || speed>=(pump-0.02))
            {
                digitalWrite(8, HIGH);
            }
            else
...
void loop()
{
  ...
  if (buttonstate == HIGH){ //checks if main switch is on
    digitalWrite(10, LOW);
  ...
  if (buttonstate == LOW){ //checks if main switch is off
    lcd.setCursor(0, 0);
    lcd.print("                ");
    lcd.setCursor(0, 1);
    lcd.print("                ");
